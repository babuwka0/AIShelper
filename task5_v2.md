# Mermaid diagrams for task 5 - redesigned fault-tolerant infrastructure

Ниже приведена новая, доработанная версия задания 5 для сети `«Зубич»`.

Вариант спроектирован с учетом условий задачи:

- бюджет проекта не ограничен;
- критичные модели оборудования должны быть официально доступны в России;
- программное обеспечение и операционные системы подбираются из российского или официально поставляемого в РФ стека;
- архитектура должна переживать не только обычные отказы, но и катастрофический сценарий полной потери основной серверной;
- в проект включены инженерные системы серверной: питание, охлаждение, пожаротушение, мониторинг среды и физическая защита.

Ключевая идея проекта: **не строить одну "серверную комнату", а строить две площадки**:

1. **Основная площадка (Primary Site)** - в головном офисе.
2. **Резервная площадка (DR Site)** - в отдельном здании или в коммерческом российском ЦОД на территории РФ.

Это позволяет сохранить работоспособность даже при сценариях уровня:

- пожар в серверной;
- длительное отключение электропитания;
- затопление;
- отказ климатической системы;
- физическое уничтожение части оборудования;
- полная потеря основной площадки.

---

## 1. Архитектура информационной инфраструктуры

Диаграмма показывает:

- все площадки организации;
- разделение на основную и резервную ИТ-площадки;
- прикладные сервисы;
- защищенные каналы связи;
- внешнее резервное хранилище копий.

```mermaid
flowchart LR
    subgraph users["Пользовательские площадки"]
        subgraph clinic1["Клиника 1"]
            c1Users["АРМ врачей и администраторов"]
            c1Xray["Диагностика / снимки"]
            c1Cash["Касса / терминалы"]
        end

        subgraph clinic2["Клиника 2"]
            c2Users["АРМ врачей и администраторов"]
            c2Xray["Диагностика / снимки"]
            c2Cash["Касса / терминалы"]
        end

        subgraph lab["Зуботехническая лаборатория"]
            labUsers["АРМ техников"]
            labCad["CAD/CAM станции"]
            labStore["Склад и маркировка"]
        end
    end

    subgraph primary["Основная площадка - головной офис"]
        primaryFw["UserGate NGFW"]
        primaryCore["Eltex core / aggregation"]

        subgraph primarySrv["Основной серверный контур"]
            virtCluster["Кластер виртуализации\nAstra Linux + БРЕСТ/VMmanager"]
            dbCluster["Кластер БД\nPostgres Pro Enterprise"]
            appLayer["МИС / файловые сервисы /\nALD Pro / почта / Service Desk"]
            storageMain["Основная СХД YADRO TATLIN"]
            backupMain["RuBackup"]
        end
    end

    subgraph dr["Резервная площадка / DR Site"]
        drFw["UserGate NGFW"]
        drCore["Eltex core / aggregation"]

        subgraph drSrv["Резервный серверный контур"]
            drVirt["Резервный кластер виртуализации"]
            drDb["Резервная реплика БД"]
            drStorage["Резервная СХД"]
            drBackup["Изолированный backup-контур"]
        end
    end

    subgraph offsite["Внешний контур хранения"]
        objectStore["S3-совместимое объектное хранилище\nв РФ / TATLIN.OBJECT"]
        tapeSafe["Офлайн-копии / ленты / сейф"]
    end

    vpn["Защищенные VPN-каналы"]
    internet["Интернет"]

    c1Users --> vpn
    c1Xray --> vpn
    c1Cash --> vpn

    c2Users --> vpn
    c2Xray --> vpn
    c2Cash --> vpn

    labUsers --> vpn
    labCad --> vpn
    labStore --> vpn

    vpn --> primaryFw
    vpn --> drFw

    primaryFw --> primaryCore
    primaryCore --> virtCluster
    primaryCore --> dbCluster
    primaryCore --> appLayer
    primaryCore --> storageMain
    primaryCore --> backupMain

    drFw --> drCore
    drCore --> drVirt
    drCore --> drDb
    drCore --> drStorage
    drCore --> drBackup

    storageMain -. репликация .-> drStorage
    dbCluster -. репликация .-> drDb
    backupMain -. копии .-> drBackup
    backupMain -. offsite copy .-> objectStore
    drBackup -. offline export .-> tapeSafe

    primaryFw --> internet
    drFw --> internet
```

---

## 2. Детальная схема серверной и инженерной инфраструктуры

Эта схема отражает не только ИТ-оборудование, но и то, без чего серверная в реальном проекте не живет:

- двухлучевое питание;
- ИБП;
- дизель-генератор;
- кондиционирование по схеме `N+1`;
- газовое пожаротушение;
- контроль доступа и датчики среды.

```mermaid
flowchart TB
    subgraph building["Основная серверная"]
        mainsA["Ввод питания A"]
        mainsB["Ввод питания B"]
        ats["АВР"]
        dg["Дизель-генератор"]
        upsA["Онлайн ИБП A"]
        upsB["Онлайн ИБП B"]
        pduA["Стоечные PDU A"]
        pduB["Стоечные PDU B"]

        cool1["Прецизионный кондиционер 1"]
        cool2["Прецизионный кондиционер 2"]
        env["Датчики температуры / влажности / протечки"]

        fireDetect["Пожарная сигнализация"]
        gasFire["Газовое пожаротушение"]
        access["СКУД + видеонаблюдение"]

        subgraph racks["ИТ-стойки"]
            fw["NGFW"]
            core["Ядро сети"]
            hosts["3-4 узла кластера виртуализации"]
            db["2 узла кластера БД"]
            san["Основная СХД"]
            backup["Backup-сервер + backup storage"]
            mgmt["Мониторинг / NMS / логирование"]
        end
    end

    mainsA --> ats
    mainsB --> ats
    dg --> ats
    ats --> upsA
    ats --> upsB
    upsA --> pduA
    upsB --> pduB

    pduA --> fw
    pduB --> fw
    pduA --> core
    pduB --> core
    pduA --> hosts
    pduB --> hosts
    pduA --> db
    pduB --> db
    pduA --> san
    pduB --> san
    pduA --> backup
    pduB --> backup
    pduA --> mgmt
    pduB --> mgmt

    cool1 --> racks
    cool2 --> racks
    env --> mgmt
    fireDetect --> gasFire
    access --> mgmt
```

---

## 3. Схема прикладного и системного стека

Здесь показаны сервисы, которые разумно использовать без VMware / Cisco и без завязки на санкционный стек.

```mermaid
flowchart LR
    subgraph userspace["Пользовательские сервисы"]
        mis["МИС стоматологии"]
        labSys["Лабораторная система"]
        office["Р7-Офис"]
        mail["МойОфис Почта"]
        files["Корпоративное файловое хранилище"]
        helpdesk["Naumen Service Desk"]
    end

    subgraph platform["Системная платформа"]
        os["Astra Linux Server"]
        virt["ПК СВ БРЕСТ / VMmanager"]
        directory["ALD Pro"]
        db["Postgres Pro Enterprise"]
        backup["RuBackup"]
        sec["UserGate + Kaspersky / MaxPatrol VM\n(по необходимости)"]
        mon["Zabbix / on-prem мониторинг"]
        siem["Kaspersky KUMA / журналирование ИБ"]
    end

    mis --> db
    labSys --> db
    files --> backup
    mis --> backup
    labSys --> backup
    mail --> backup
    helpdesk --> backup
    office --> files
    mail --> directory
    helpdesk --> directory
    mis --> files
    labSys --> files

    os --> virt
    virt --> db
    virt --> directory
    virt --> backup
    virt --> mon
    sec --> siem
    sec --> mis
    sec --> mail
    sec --> files
```

---

## 4. Сценарий аварийного восстановления при полной потере основной площадки

Самая важная схема для этой задачи: что происходит, если основная серверная перестала существовать полностью.

```mermaid
flowchart TB
    start(["Катастрофа: основная серверная уничтожена"])
    detect["Фиксация инцидента\nмониторингом и ИТ-службой"]
    isolate["Подтверждение полной недоступности\nPrimary Site"]
    decision{"DR-площадка доступна?"}

    activateDr["Переключение на DR Site"]
    promoteDb["Подъем резервной БД\nкак основной"]
    startApps["Запуск критичных ВМ\nМИС, AD/ALD, файлов, кассы"]
    reroute["Перенастройка VPN/DNS/маршрутов"]
    workDegraded["Работа в деградированном,\nно продуктивном режиме"]

    restoreFromOffsite["Восстановление из offsite-копий\nили офлайн-архива"]
    buildNewSite["Развертывание новой основной площадки"]
    reverseSync["Обратная репликация данных"]
    returnPrimary["Возврат к двухплощадочному режиму"]
    endNode(["Нормализованный режим после восстановления"])

    start --> detect --> isolate --> decision
    decision -->|"Да"| activateDr --> promoteDb --> startApps --> reroute --> workDegraded
    decision -->|"Нет"| restoreFromOffsite --> buildNewSite --> startApps
    workDegraded --> buildNewSite --> reverseSync --> returnPrimary --> endNode
```

---

## 5. Режимы функционирования бизнес-процессов

Для проекта удобно выделить четыре режима, а не три:

1. **Оперативный режим**  
   Повседневная работа клиник, лаборатории, кассы, регистратуры, диагностики.

2. **Плановый режим**  
   Ночные резервные копии, обновления, отчеты, профилактика, тест восстановления.

3. **Деградированный режим**  
   Работа через резервную площадку, при пониженной производительности и отключении второстепенных сервисов.

4. **Аварийно-восстановительный режим**  
   Отработка катастрофы, запуск DR-площадки, восстановление из offsite и офлайн-копий.

```mermaid
flowchart LR
    ops["Оперативный режим"]
    plan["Плановый режим"]
    degraded["Деградированный режим"]
    drmode["Аварийно-восстановительный режим"]

    ops --> plan
    ops --> degraded
    degraded --> drmode
    drmode --> ops
```

---

## 6. Практические проектные решения, которые обязательно стоит указать в пояснении

### 6.1. Защита от потери площадки

- Основная и резервная площадки должны быть территориально разнесены.
- Резервная площадка не должна находиться в том же здании, что и основная серверная.
- Минимум одна копия данных должна храниться **вне основной площадки**.
- Минимум одна копия должна быть **неизменяемой** или **офлайн**, чтобы защититься от шифровальщиков и ошибок администратора.

### 6.2. Защита от отказа интернет-магистрали и провайдера

Для головного офиса, серверной и DR Site недостаточно иметь "просто интернет". В проекте должны быть предусмотрены **минимум два независимых провайдера связи**:

- **основной провайдер** - выделенный проводной канал связи с SLA;
- **резервный провайдер** - другой оператор связи с максимально независимой физической инфраструктурой;
- **аварийный третий контур** - LTE/5G-канал через отдельного мобильного оператора для минимальной работоспособности при разрушении проводной инфраструктуры.

Ключевое требование: резервный канал не должен идти по той же трассе, что и основной. Для этого в проекте фиксируются следующие условия:

- разные точки ввода кабеля в здание;
- разные стояки и кабельные каналы;
- разные городские узлы связи;
- минимум общих участков магистральной сети;
- по возможности разные вышестоящие транспортные маршруты.

На практике это можно описать так:

- **Основной оператор** - например, Ростелеком Бизнес, выделенная ВОЛС;
- **Резервный оператор** - например, МТС Бизнес, МегаФон или ЭР-Телеком Бизнес, с отдельной трассой и отдельным узлом доступа;
- **Аварийный мобильный канал** - LTE/5G через мобильного оператора, не совпадающего с основным проводным поставщиком.

Переключение между каналами должно выполняться на пограничных устройствах UserGate:

- основной WAN используется в нормальном режиме;
- резервный WAN активируется автоматически при деградации или потере основного;
- VPN-туннели между площадками настраиваются по обоим провайдерам;
- при отказе одного оператора связь филиалов и DR Site сохраняется через резервный контур;
- после восстановления магистрали возможен автоматический или регламентный возврат на основной канал.

Для снижения последствий аварии на городской магистрали в проекте также рекомендуется:

- размещать DR Site в другом здании или ЦОД с собственными операторами связи;
- резервировать каналы не только у головного офиса, но и у DR Site;
- регулярно тестировать автоматическое переключение на резерв;
- вести мониторинг качества каналов и оповещение ИТ-службы при росте потерь, задержек и полной недоступности;
- обеспечивать питание операторского оборудования и терминалов связи от ИБП.

### 6.3. Политика резервного копирования

Рекомендуется схема `3-2-1-1-0`:

- **3** копии данных;
- **2** разных типа носителей;
- **1** копия вне площадки;
- **1** копия офлайн или immutable;
- **0** ошибок при проверке восстановления.

Для проекта это можно реализовать так:

- основная рабочая копия - на основной СХД;
- backup-копия - на backup-хранилище в основной серверной;
- реплика backup - на DR-площадку;
- immutable/offsite - в S3-совместимое хранилище в РФ;
- офлайн-экспорт наиболее критичных архивов - на ленты или съемные носители в сейфе.

### 6.4. Питание

- два независимых ввода электропитания;
- АВР;
- онлайн ИБП с двойным преобразованием;
- время автономии до запуска ДГУ;
- дизель-генератор для длительной автономной работы;
- раздельные линии `A/B` до стоек и устройств с двумя блоками питания.

### 6.5. Охлаждение

- отдельная серверная без совмещения с бытовыми помещениями;
- прецизионное кондиционирование;
- резервирование `N+1`;
- поддержание температуры и влажности в допустимом диапазоне;
- датчики температуры, влажности и протечки;
- запрет на обычные бытовые сплит-системы как единственный способ охлаждения критичной серверной.

### 6.6. Пожаротушение

- раннее обнаружение дыма и перегрева;
- автоматическая пожарная сигнализация;
- газовое пожаротушение, безопасное для серверного оборудования;
- автоматическое отключение питания в сценарии пожара по утвержденному проекту;
- регламент допуска персонала после срабатывания.

### 6.7. Сетевой и информационный периметр

- NGFW на основной и резервной площадках;
- site-to-site VPN для филиалов;
- отдельные VLAN для врачей, администраторов, касс, Wi-Fi, диагностики, серверов и управления;
- журналирование событий безопасности;
- централизованный мониторинг;
- регулярная проверка восстановления резервных копий.

---

## 7. Что брать в текстовую часть проекта

Если эту версию нужно использовать как основу для сдачи, в текстовую часть стоит включить:

- обоснование двухплощадочной архитектуры;
- обоснование выбора российского и официально доступного в РФ стека;
- перечень критичных ИС и их приоритет восстановления;
- параметры `RPO/RTO` хотя бы в учебном виде;
- описание резервного копирования и DR-сценария;
- инженерное обеспечение серверной;
- таблицу оборудования из `table5.md`.

---

## 8. Короткий вывод

Для этой задачи лучшее решение при неограниченном бюджете - не "самая дорогая серверная", а **отказоустойчивая распределенная инфраструктура**:

- основная площадка;
- резервная площадка;
- российский или официально доступный в России стек оборудования и ПО;
- резервное копирование по схеме `3-2-1-1-0`;
- инженерная защита серверной;
- готовый сценарий работы даже при полном уничтожении основной серверной.
